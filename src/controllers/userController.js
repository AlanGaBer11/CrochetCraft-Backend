...
const getAllUsers = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ successs: false, message: 'Error Interno Del Servidor' })
  }
}
...
const createUser = async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ successs: true, message: 'Usuario Creado', newUser })
  } catch (error) {
...
const deleteUser = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: 'Usuario Eliminado', deletedUser })
  } catch (error) {
...
const getProfile = async (req, res) => {
  try {
    ...
    const profile = await userProcess.getUserProfile(userId)
    res.status(200).json({
      success: true,
...
const updateProfile = async (req, res) => {
  try {
    const userId = req.user.id
    const updatedProfile = await userProcess.updateUserProfile(
      userId,
      req.body
    )
    res.status(200).json({
